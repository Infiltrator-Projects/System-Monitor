...
**Current source version:** 1.0.74 ([version file](support/VERSION))\
...
The Windows preview uses the same product hierarchy and Infiltratr visual contract as the Linux application rather than raw Win32 control layout. The eight main pages are top tabs; the left rail belongs inside Performance for resource selection. CPU and Memory now use the same side-rail pattern as Linux, including compact live history sparklines and their distinct canonical resource colours. The Windows shell consumes Common 1.19.24's exact Day/Night palette roles and design metrics; View → Theme provides Follow system, Day and Night, with the choice persisted per Windows user. Night remains the default for the preview to preserve the established black/graphite/cyan product face until the user explicitly chooses otherwise. MB Corpo family names are requested when already installed, with an explicit Segoe UI fallback and no font redistribution. Processes uses the read-only Tool Help/Win32 backend with SID-derived ownership and now receives themed row/header drawing rather than stock common-control chrome. App History, Startup Apps, Users, Details, Services and File Systems remain explicit placeholders until their native Windows backends are implemented. Windows process control, command-line/GPU enrichment and device collectors remain unavailable, and the Linux desktop remains the feature-complete product.
