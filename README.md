...
**Current source version:** 1.0.81 ([version file](support/VERSION))\
...
The Windows preview now uses the same application-facing Performance contract and shared view-model layer as Linux while retaining native Win32/GDI rendering and native Windows collectors. CPU and memory remain live through Win32/PSAPI. Physical disks are discovered with storage IOCTLs and expose identity, capacity, live throughput, activity, response and queue data plus mapped volume/filesystem usage. Network adapters are discovered through IP Helper and expose addresses, MAC identity, connection state, link speed, cumulative traffic and live send/receive utilisation. Graphics adapters are identified natively; unsupported GPU utilisation/temperature telemetry remains explicit `N/A` rather than being guessed. The Performance rail now enumerates every discovered disk, network adapter and GPU, retains type-plus-device-index selection, and scrolls when the native device set exceeds the visible rail. CPU/Memory keep the richer canonical graph/detail composition; device pages consume toolkit-neutral disk/network/GPU titles, summaries and formatted metrics from `performance_view.[ch]`. View → Theme continues to provide Follow system, Day and Night using Common 1.19.24's palette roles. Processes remains read-only, and the other Windows product pages remain explicit placeholders until their native backends are implemented.
