...
**Current source version:** 1.0.73 ([version file](support/VERSION))\
...
The Windows preview now uses the same product hierarchy and Infiltratr visual contract as the Linux application instead of raw Win32 control layout. The eight main pages are top tabs; the left rail belongs inside Performance for CPU/Memory resource selection. The Windows shell uses Common 1.19.24's Night palette and design metrics, including the #050608 canvas, graphite panel/card/surface layers, #00ADEF cyan selection/accent treatment, rounded cards and the Common typography fallbacks. Performance is populated from the existing Win32/PSAPI CPU and memory backend with live history graphs and headline summary values, and Processes uses the read-only Tool Help/Win32 process backend with SID-derived ownership. App History, Startup Apps, Users, Details, Services and File Systems remain explicit placeholders until their native Windows backends are implemented. Windows process control, command-line/GPU enrichment and device collectors remain unavailable, and the Linux desktop remains the feature-complete product.
