...
If GitHub private vulnerability reporting is available for this repository, use the repository's **Security** reporting flow. Otherwise, report the issue privately to `infiltratr@yandex.com` with the subject `System Monitor security report`.
...
The project does not treat normal absence of privileged telemetry as a vulnerability. System Monitor deliberately has no project-owned privileged helper or daemon; optional information that the current user cannot read should degrade to unavailable.

The maintained security and trust model, including the Bluetooth capability boundary and treatment of native data as external input, is documented in [ARCHITECTURE.md](../docs/ARCHITECTURE.md).
