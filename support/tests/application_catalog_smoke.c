...
int main(void)
{
    ...
    const LsmApplicationEntry *flatpak_service =
        lsm_application_catalog_lookup_cgroup(
            catalog,
            "/user.slice/user-1000.slice/user@1000.service/app.slice/"
            "app-flatpak-org.example.Writer@4815.service");
    const LsmApplicationEntry *gnome_scope =
        lsm_application_catalog_lookup_cgroup(
            catalog,
            "/user.slice/user-1000.slice/user@1000.service/app.slice/"
            "app-gnome-org.example.Writer-a1b2c3.scope");
    const bool okay =
        ...
        flatpak_service == flatpak &&
        gnome_scope == flatpak;

    lsm_application_catalog_destroy(catalog);
    remove_tree(root);
    puts("XDG application catalogue, cgroup identity and override policy passed.");
    return okay ? 0 : 9;
}
