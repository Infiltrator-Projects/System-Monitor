...
static bool tree_has_suffix(const char *path, const char *suffix)
{
    struct stat status;
    if (!path || !suffix || lstat(path, &status) != 0) return false;
    ...
    while (!found && (entry = readdir(directory))) {
        ...
        if (join_path(child, sizeof(child), path, entry->d_name))
            found = tree_has_suffix(child, suffix);
    }
    closedir(directory);
    return found;
}
...
static void run_make(const char *make_path, const char *source_root,
                     ...
                     const char *cflags, const char *profile_ldflags)
{
    char job_text[32];
    ...
    char profile_ldflags_assignment[LSM_BUILDER_TEXT_LEN];
    const char *arguments[LSM_BUILDER_MAX_ARGS];
    size_t count = 0U;
    ...
    append_make_argument(arguments, &count, profile_assignment,
                         sizeof(profile_assignment), "BUILD_PROFILE", profile);
    if (cflags)
        append_make_argument(arguments, &count, cflags_assignment,
                             sizeof(cflags_assignment), "CFLAGS", cflags);
    if (profile_ldflags)
        append_make_argument(arguments, &count, profile_ldflags_assignment,
                             sizeof(profile_ldflags_assignment),
                             "NATIVE_PROFILE_LDFLAGS", profile_ldflags);
...
int main(int argc, char **argv)
{
    ...
    const bool aggressive_profile = strcmp(profile, "aggressive") == 0;
    if (aggressive_profile) {
        const char *pgo_generate_probe[] = {
            "-fprofile-generate", "-fprofile-update=atomic"
        };
        const char *pgo_use_probe[] = {
            "-fprofile-use", "-fprofile-correction",
            "-fprofile-partial-training", "-Wno-missing-profile"
        };
        if (!probe_compiler(compiler, pgo_generate_probe,
                            sizeof(pgo_generate_probe) /
                                sizeof(pgo_generate_probe[0])) ||
            !probe_compiler(compiler, pgo_use_probe,
                            sizeof(pgo_use_probe) /
                                sizeof(pgo_use_probe[0])))
            fail("aggressive profile requires compiler-supported two-pass PGO");
    }
    ...
    printf("  Jobs:         %d\n", jobs);
    if (aggressive_profile)
        puts("  PGO:          two-pass training on this machine before final rebuild");
    ...
    if (run_tests) {
        run_make(make_path, source_root, jobs, compiler, pkg_config,
                 "build-check", profile, NULL, NULL);
        run_required(source_root, clean_arguments);
    }

    if (aggressive_profile) {
        char profile_directory[LSM_BUILDER_PATH_LEN];
        if (!join_path(profile_directory, sizeof(profile_directory),
                       cleanup_probe, "pgo"))
            fail("PGO profile path is too long");
        make_directories(profile_directory, 0700);

        char generate_flags[LSM_BUILDER_TEXT_LEN];
        written = snprintf(generate_flags, sizeof(generate_flags),
                           "%s -fprofile-generate=%s -fprofile-update=atomic",
                           cflags, profile_directory);
        if (written < 0 || (size_t)written >= sizeof(generate_flags))
            fail("PGO generation flags are too long");

        char generate_ldflags[LSM_BUILDER_TEXT_LEN];
        written = snprintf(generate_ldflags, sizeof(generate_ldflags),
                           "-fprofile-generate=%s -fprofile-update=atomic",
                           profile_directory);
        if (written < 0 || (size_t)written >= sizeof(generate_ldflags))
            fail("PGO generation link flags are too long");

        puts("\nBuilding instrumented aggressive binary for PGO training...");
        run_make(make_path, source_root, jobs, compiler, pkg_config,
                 "build/system-monitor", profile, generate_flags,
                 generate_ldflags);

        char training_binary[LSM_BUILDER_PATH_LEN];
        if (!join_path(training_binary, sizeof(training_binary), source_root,
                       "build/system-monitor"))
            fail("PGO training binary path is too long");
        const char *const training_arguments[] = {
            training_binary, "--pgo-train", NULL
        };
        puts("Running representative native collector/process workload...");
        run_required(source_root, training_arguments);
        if (!tree_has_suffix(profile_directory, ".gcda"))
            fail("PGO training completed without producing profile data");

        run_required(source_root, clean_arguments);
        char use_flags[LSM_BUILDER_TEXT_LEN];
        written = snprintf(use_flags, sizeof(use_flags),
                           "%s -fprofile-use=%s -fprofile-correction "
                           "-fprofile-partial-training -Wno-missing-profile",
                           cflags, profile_directory);
        if (written < 0 || (size_t)written >= sizeof(use_flags))
            fail("PGO use flags are too long");

        char use_ldflags[LSM_BUILDER_TEXT_LEN];
        written = snprintf(use_ldflags, sizeof(use_ldflags),
                           "-fprofile-use=%s -fprofile-correction "
                           "-fprofile-partial-training -Wno-missing-profile",
                           profile_directory);
        if (written < 0 || (size_t)written >= sizeof(use_ldflags))
            fail("PGO use link flags are too long");

        puts("Rebuilding aggressive binary using measured PGO profile...");
        run_make(make_path, source_root, jobs, compiler, pkg_config,
                 "build/system-monitor", profile, use_flags, use_ldflags);
    } else {
        run_make(make_path, source_root, jobs, compiler, pkg_config,
                 "build/system-monitor", profile, cflags, NULL);
    }
    ...
    written = snprintf(text, sizeof(text),
        ...
        aggressive_profile ? "trained two-pass profile" : "disabled",
        common_version);
    ...
    run_make(make_path, source_root, jobs, compiler, pkg_config,
             "build/build-deb-package", profile, NULL, NULL);
