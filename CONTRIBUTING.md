...
`make check` is the authoritative executed verification suite. CTest remains available for targeted local regression debugging, but CI does not replay the complete smoke suite through both build systems. Use `make docs` for Doxygen validation and `make -j2 release` for a release-equivalent packaging pass. Do not use direct `make install`.
