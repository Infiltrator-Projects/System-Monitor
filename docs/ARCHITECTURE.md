...
A completed process or hardware sample is published as a coherent unit. Presentation code may display metrics with different collection cadences, but a value is not reported as newly available until the collector has established it for that sample. Monitor backends attach a non-zero completed-sample generation and monotonic completion timestamp only after a native collection cycle has finished. The bounded Overview history accepts each generation once, inserts an explicit missing sample when generations are skipped, and is fed by the always-running monitor path rather than by Overview visibility. A slow asynchronous Linux sample therefore cannot be mistaken for repeated flat-line measurements. Topology generations and stable identities distinguish device replacement from ordinary metric refresh; retained Overview device identities are resolved again against current topology before navigation.
...
Application startup follows the same rule: first paint is not held hostage by unrelated page construction. Performance is the only notebook page constructed before the window is shown. Other pages are built on first navigation, and recurring page-specific inventories do not install timers until their owning page exists. Once Services, Users or File Systems has been constructed, its timer remains a cheap scheduler hook while hidden and performs the underlying inventory only when that tab is active; navigation performs the immediate refresh when the user returns. Persistent App History accounting is deliberately different from its lazy GTK page: its non-visual model starts after the first frame and consumes process snapshots for the whole application lifetime, so history does not begin only when the user first opens the tab. Processes and Details use the foreground process cadence; Overview and unrelated tabs use the lower background process cadence while still feeding App History. Startup-application discovery and durable XDG override writes are dispatched through workers; completed plain-data results are the only startup inventory state published back onto GTK.

Performance sampling and Performance presentation are intentionally different operations. Each completed monitor generation is appended once to every retained graph history, including hidden resources, but GTK label formatting, style changes and detail-table updates are limited to the selected Performance page and the mapped entries in its current device rail. Overview likewise tracks separate monitor and process generations so one source cannot force the unchanged half of the dashboard to redraw. Worker request queues are coalescing: a periodic request that duplicates work already in flight does not create a catch-up loop after a slow sample.
...
Common's target is reference-quality, leading-edge and complete reusable code, not merely a lowest-common-denominator helper set. Common 1.19.34 owns the toolkit-neutral POSIX user/XDG path and recursive-directory contracts, deterministic ASCII classification/case-matching/ordering, stable non-cryptographic FNV-1a mixing, monotonic unsigned-counter delta/rate mechanics and normalized absolute POSIX-clock deadline conversion used by System Monitor. Signature composition, Linux counter identity/reset/wrap policy, pthread synchronization policy, GTK/GLib-specific presentation and object-lifecycle policy remain local. If System Monitor contains a stronger implementation of a capability that is fundamentally generic, the correct direction is to improve Common so that its generic contract preserves the local implementation's correctness, performance, resilience and useful capabilities. Once Common is at least as strong, System Monitor should use Common and remove the duplicate implementation.
