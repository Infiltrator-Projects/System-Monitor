...
The first Windows implementation now consists of `src/main_windows.c`, `src/monitor_backend_windows.c` and `src/process_backend_windows.c`. The Win32 presentation is intentionally a thin platform adapter over the same plain-C backend contracts rather than a fork of the Linux application. It follows the same hierarchy as Linux: the eight product areas are top tabs, while Performance owns its own resource rail. The shell consumes Common 1.19.24's documented Day/Night palette and design metrics directly in native GDI drawing and mirrors the Linux Performance-page composition: compact title/scale headers, primary graphs, the canonical resource rail, metric/detail panels and the Memory composition section. Unsupported native telemetry stays present as `N/A` presentation state rather than causing Windows-specific layout collapse. CPU and Memory are the first Performance resources and carry live history graphs; Processes remains a native read-only list. App History, Startup Apps, Users, Details, Services and File Systems remain explicit placeholders until their native Windows backends are implemented. The monitor slice covers aggregate CPU utilisation, logical processor count, uptime, system process/thread/handle totals and physical/commit memory through Win32/PSAPI. The process slice is read-only: Tool Help provides PID/name/parent/thread inventory, access-token SIDs provide stable account identity and current-user ownership, and permission-dependent Win32 queries provide creation identity, CPU time, working set, I/O, priority, executable path and handle count with retained PID-reuse-safe rate baselines. Command lines, GPU/cgroup fields and every process-control operation remain unsupported. Unsupported fields stay unavailable rather than being inferred from Linux semantics.

GTK 3 remains the Linux presentation toolkit. The Windows preview uses native Win32/GDI/common-controls presentation, avoiding a GTK runtime dependency on Windows while preserving the shared design contract. The Windows adapter resolves the same Common Day/Night palette roles and design metrics into native drawing, uses the Windows application-theme setting for Follow system, persists only the platform-specific theme selection in HKCU, and falls back explicitly to Segoe UI when the preferred Common typography family is not installed. Reusable accounting, parsing and model layers must not depend on either presentation toolkit.
