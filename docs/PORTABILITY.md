...
The first Windows implementation now consists of `src/main_windows.c`, `src/monitor_backend_windows.c` and `src/process_backend_windows.c`. The Win32 presentation is intentionally a thin platform adapter over the same plain-C backend contracts rather than a fork of the Linux application. It exposes the complete top-level navigation shell first: Performance and Processes are populated, while App History, Startup Apps, Users, Details, Services and File Systems remain explicit placeholders until their native Windows backends are implemented. The monitor slice covers aggregate CPU utilisation, logical processor count, uptime, system process/thread/handle totals and physical/commit memory through Win32/PSAPI. The process slice is read-only: Tool Help provides PID/name/parent/thread inventory, access-token SIDs provide stable account identity and current-user ownership, and permission-dependent Win32 queries provide creation identity, CPU time, working set, I/O, priority, executable path and handle count with retained PID-reuse-safe rate baselines. Command lines, GPU/cgroup fields and every process-control operation remain unsupported. Unsupported fields stay unavailable rather than being inferred from Linux semantics.

GTK 3 remains the Linux presentation toolkit. The Windows preview uses the native Win32 user-interface and common-controls APIs, avoiding a GTK runtime dependency on Windows. Reusable accounting, parsing and model layers must not depend on either presentation toolkit.
