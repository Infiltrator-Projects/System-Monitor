...
GTK 3 remains the Linux presentation toolkit. The Windows preview uses native Win32/GDI/common-controls presentation, avoiding a GTK runtime dependency on Windows while preserving the shared design contract. Reusable accounting, parsing and model layers must not depend on either presentation toolkit.
