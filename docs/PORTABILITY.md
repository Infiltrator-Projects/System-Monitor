...
Time used for rates, refresh cadence and retry deadlines is elapsed time and therefore uses monotonic clocks. Canonical persisted timestamps also remain Unix/SI values. Human-facing civil timestamps are presentation only: when the validated temporal-v3 provider from Infiltrator System Settings is available they use that policy through Common 1.19.25, including extended clock systems and configured geographic context; otherwise the native OS locale remains authoritative.
