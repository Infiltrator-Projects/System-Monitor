                             ...
                             const char *fallback)
{
    ...
    if (clean_vendor[0] && clean_product[0] &&
        !lsm_ascii_starts_with_ci(clean_product, clean_vendor)) {
...
static bool collect_mount(const LsmMountInfoEntry *entry, void *user_data)
{
    ...
    if (lsm_ascii_equal_ci(entry->filesystem, "vfat") ||
        lsm_ascii_equal_ci(entry->filesystem, "msdos")) {
        ...
        if (filesystem_label_from_device_database(
            ...
            lsm_ascii_starts_with_ci(precise_filesystem, "FAT"))
...
static bool cpu_sensor_name(const char *name)
{
    ...
    for (size_t index = 0U; index < LSM_ARRAY_LENGTH(sources); index++)
        if (lsm_ascii_equal_ci(name, sources[index])) return true;
    return false;
}
...
static int temperature_label_score(const char *label)
{
    ...
    if (lsm_ascii_contains_ci(label, "package") || lsm_ascii_contains_ci(label, "tdie") ||
        lsm_ascii_contains_ci(label, "tctl") || lsm_ascii_contains_ci(label, "cpu") ||
        lsm_ascii_contains_ci(label, "composite"))
        return 100;
    if (lsm_ascii_contains_ci(label, "core")) return 30;
    return 10;
}
