 ...
 *
 * Common owns binary memory/storage presentation, decimal network units and
 * optional scalar formatting. System Monitor keeps only its two-decimal GHz
 * convenience adapter here.
 *
...
static inline char *lsm_metric_format_ghz(bool available, double value,
                                          char *buffer, size_t size)
{
    ...
    (void)infiltratr_format_scalar(
        available, (long double)value, &options, buffer, size);
    return buffer;
}
