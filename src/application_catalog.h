 ...
 */
#ifndef INFILTRATOR_SYSTEM_MONITOR_APPLICATION_CATALOG_H
#define INFILTRATOR_SYSTEM_MONITOR_APPLICATION_CATALOG_H
...
/**
 * Resolve a standard desktop application unit embedded in a cgroup-v2 path.
 *
 * The parser recognises the cross-desktop app[-launcher]-ApplicationID
 * service/scope convention and treats it only as an additional metadata hint.
 * Ordinary executable/ancestor matching remains available as a fallback.
 *
 * @param [in] catalog Catalogue to query.
 * @param [in] cgroup_path Unified cgroup-v2 path from /proc/PID/cgroup.
 * @return Catalogue-owned immutable entry, or NULL when no standard
 *         application unit maps to a desktop ID.
 */
