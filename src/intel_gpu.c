                          ...
                          double divisor, double scale, double *result)
{
    ...
    uint64_t value_delta = 0U;
    uint64_t enabled = 0U;
    uint64_t running = 0U;
    if (!counter->initialised ||
        !lsm_u64_counter_delta(
            sample.value, counter->previous_value, &value_delta) ||
        !lsm_u64_counter_delta(
            sample.time_enabled, counter->previous_enabled, &enabled) ||
        !lsm_u64_counter_delta(
            sample.time_running, counter->previous_running, &running)) {
...
{
    return driver && (lsm_ascii_equal_ci(driver, "i915") ||
                      lsm_ascii_equal_ci(driver, "xe"));
}
