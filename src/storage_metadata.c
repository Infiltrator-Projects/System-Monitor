...
{
    if (!metadata ||
        (!lsm_ascii_equal_ci(metadata->filesystem, "vfat") &&
         !lsm_ascii_equal_ci(metadata->filesystem, "msdos")))
        return NULL;
    if (lsm_ascii_equal_ci(metadata->version, "FAT12")) return "FAT12";
    if (lsm_ascii_equal_ci(metadata->version, "FAT16")) return "FAT16";
    if (lsm_ascii_equal_ci(metadata->version, "FAT32")) return "FAT32";
    return NULL;
}
...
static const char *partition_role(const char *partition_type)
{
    ...
    for (size_t index = 0U;
         ...
         index++) {
        if (lsm_ascii_equal_ci(partition_type, gpt_partition_roles[index].guid))
