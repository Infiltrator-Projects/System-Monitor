...
static void address_string(const LsmBluetoothAddress *address,
...
LsmBluetoothTrafficStartResult lsm_bluetooth_traffic_start(void)
{
    ...
    int descriptor = socket(
        AF_BLUETOOTH, SOCK_RAW | SOCK_CLOEXEC, LSM_BTPROTO_HCI);
    bool bound = false;
    if (descriptor >= 0) {
        ...
        LsmSockaddrHci address;
        memset(&address, 0, sizeof(address));
        address.family = AF_BLUETOOTH;
        address.device = LSM_HCI_DEV_NONE;
        address.channel = LSM_HCI_CHANNEL_MONITOR;
    ...
    if (!lsm_linux_drop_all_capabilities()) {
...
bool lsm_bluetooth_traffic_refresh_connections(const char *controller)
{
    ...
    const size_t allocation =
        sizeof(LsmHciConnectionList) +
        LSM_HCI_MAX_CONNECTIONS * sizeof(LsmHciConnectionInfo);
    LsmHciConnectionList *list = calloc(1U, allocation);
    if (!list) {
        (void)close(descriptor);
        return false;
    }
    list->device_id = index;
    list->connection_count = LSM_HCI_MAX_CONNECTIONS;
    const bool okay = ioctl(descriptor, LSM_HCIGETCONNLIST, list) == 0;
    (void)close(descriptor);
    if (!okay) {
        free(list);
        return false;
    ...
    for (uint16_t connection = 0U;
         connection < list->connection_count; connection++) {
        const LsmHciConnectionInfo *info = &list->connections[connection];
        LsmBluetoothLinkCounter *slot = link_for_handle_locked(
            index, info->handle);
        if (!slot) continue;
        char remote[32] = {0};
        address_string(&info->address, remote, sizeof(remote));
        if (slot->address[0] &&
            !lsm_ascii_equal_ci(slot->address, remote)) {
            slot->rx_bytes = 0U;
            slot->tx_bytes = 0U;
        }
    ...
    LsmBluetoothTrafficCounters *counters)
{
    ...
    if (available) {
        for (size_t slot_index = 0U; slot_index < LSM_HCI_LINK_SLOTS;
            ...
            if (!slot->used || slot->controller_index != index ||
                !slot->address[0] ||
                !lsm_ascii_equal_ci(slot->address, address))
                continue;
