...
static void startup_refresh_result_free(gpointer data)
{
    StartupRefreshResult *result = data;
    if (!result) return;
    lsm_startup_backend_free(result->entries);
    g_free(result);
}

static void startup_action_result_free(gpointer data)
{
    StartupActionResult *result = data;
    if (!result) return;
    g_free(result->id);
    g_free(result->source_identity);
    g_free(result->origin_identity);
                                       ...
                                       gpointer user_data)
{
    ...
    gtk_widget_set_sensitive(app->startup.startup_toggle_button,
                             selected && !app->startup.startup_action_pending);
    gtk_widget_set_sensitive(app->startup.startup_open_button, selected);
    if (selected && !app->startup.startup_action_pending) {
...
static void startup_action_worker(GTask *task, gpointer source_object,
                                  gpointer task_data,
                                  GCancellable *cancellable)
{
    (void)source_object;
    (void)cancellable;
    StartupActionResult *result = task_data;
    GError *error = NULL;
    if (!lsm_startup_backend_set_enabled(
            result->id, result->source_identity, result->origin_identity,
            result->enabled != FALSE, &error)) {
        result->error_message = g_strdup(
            error ? error->message : "The desktop file could not be written.");
    }
    if (error) g_error_free(error);
    g_task_return_pointer(task, result, startup_action_result_free);
}

static void startup_action_complete(GObject *source_object,
                                    ...
                                    gpointer user_data)
{
    ...
    if (!app) {
        startup_action_result_free(result);
        return;
    }

    app->startup.startup_action_pending = FALSE;
    if (result && result->error_message && !app->runtime.shutting_down)
        lsm_ui_show_error(GTK_WINDOW(app->shell.window),
                          "Unable to change startup application", "%s",
                          result->error_message);
    startup_action_result_free(result);
    if (!app->runtime.shutting_down) lsm_startup_refresh(app);
}
...
static void startup_toggle(GtkButton *button, gpointer user_data)
{
    ...
    if (!app || app->runtime.shutting_down ||
        app->startup.startup_action_pending)
        return;
    ...
    StartupActionResult *result = g_new0(StartupActionResult, 1);
    result->id = id;
    result->source_identity = source_identity;
    result->origin_identity = origin_identity;
    result->enabled = !enabled;

    app->startup.startup_action_pending = TRUE;
    gtk_widget_set_sensitive(app->startup.startup_toggle_button, FALSE);
    gtk_button_set_label(GTK_BUTTON(app->startup.startup_toggle_button),
                         "Working…");

    GTask *task = g_task_new(G_OBJECT(app->shell.window), NULL,
                             startup_action_complete, NULL);
    g_task_set_task_data(task, result, NULL);
    g_task_run_in_thread(task, startup_action_worker);
    g_object_unref(task);
}
...
static void startup_refresh_worker(GTask *task, gpointer source_object,
                                   gpointer task_data,
                                   GCancellable *cancellable)
{
    (void)source_object;
    (void)task_data;
    (void)cancellable;
    StartupRefreshResult *result = g_new0(StartupRefreshResult, 1);
    result->available =
        lsm_startup_backend_collect(&result->entries, &result->count);
    g_task_return_pointer(task, result, startup_refresh_result_free);
}

static void startup_present_refresh(LsmApp *app,
                                    const StartupRefreshResult *result)
{
    if (!app || !app->startup.startup_store || !result) return;
    ...
    for (size_t i = 0U; i < result->count; i++) {
        const LsmStartupEntry *entry = &result->entries[i];
        if (*search && !lsm_ui_text_matches(entry->name, search) &&
            ...
            !lsm_ui_text_matches(entry->id, search))
            continue;
...
static void startup_refresh_complete(GObject *source_object,
                                     GAsyncResult *async_result,
                                     gpointer user_data)
{
    (void)user_data;
    ...
    LsmApp *app = source_object
        ? g_object_get_data(source_object, "lsm-app") : NULL;
    if (!app) {
        startup_refresh_result_free(result);
        return;
    }

    const gboolean refresh_again = app->startup.startup_refresh_again;
    app->startup.startup_refresh_pending = FALSE;
    app->startup.startup_refresh_again = FALSE;

    if (!app->runtime.shutting_down && result)
        startup_present_refresh(app, result);
    startup_refresh_result_free(result);

    if (refresh_again && !app->runtime.shutting_down)
        lsm_startup_refresh(app);
}

void lsm_startup_refresh(LsmApp *app)
{
    if (!app || !app->startup.startup_store || !app->shell.window ||
        app->runtime.shutting_down)
        return;
    if (app->startup.startup_refresh_pending) {
        app->startup.startup_refresh_again = TRUE;
        return;
    }

    app->startup.startup_refresh_pending = TRUE;
    app->startup.startup_refresh_again = FALSE;
    lsm_ui_set_label_text(app->startup.startup_count_label,
                          "Refreshing startup applications…");

    GTask *task = g_task_new(G_OBJECT(app->shell.window), NULL,
                             startup_refresh_complete, NULL);
    g_task_run_in_thread(task, startup_refresh_worker);
    g_object_unref(task);
}
...
void lsm_startup_build(LsmApp *app, GtkWidget *container)
{
    ...
    gtk_box_pack_start(GTK_BOX(container), app->startup.startup_count_label,
                       FALSE, FALSE, 0);
    lsm_ui_set_label_text(app->startup.startup_count_label,
                          "Select the Startup Apps tab to load startup applications");
}
