...
{
    GError *error = NULL;
    char *value = g_key_file_get_locale_string(file, "Desktop Entry", key, NULL,
                                               &error);
    if (error) {
        g_error_free(error);
        return g_strdup("");
                                   ...
                                   gboolean user_entry, StartupEntry *entry)
{
    ...
    gboolean gnome_enabled = desktop_boolean(file,
...
static gboolean append_entry(StartupEntry **entries, size_t *count,
                             size_t *capacity, const StartupEntry *entry)
{
...
static void scan_directory(const char *directory, gboolean user_entry,
                           StartupEntry **entries, size_t *count,
                           size_t *capacity)
{
