                                       ...
                                       GError **error)
{
    ...
    const int mkdir_failure = lsm_mkdir_parents(directory, 0755U);
    if (mkdir_failure != 0) {
        g_set_error(error, G_FILE_ERROR,
                    g_file_error_from_errno(mkdir_failure),
    ...
    char target[LSM_PATH_LEN];
    if (!lsm_join_path(target, sizeof(target), directory, desktop_id)) {
        g_set_error(error, G_FILE_ERROR,
                    g_file_error_from_errno(ENAMETOOLONG),
                    "Startup override path is too long");
        g_free(data);
        return FALSE;
    }
...
char *lsm_startup_backend_location_uri(const char *source_identity)
{
    ...
    char directory[LSM_PATH_LEN];
    if (!lsm_path_dirname(source_identity, directory, sizeof(directory)))
        return NULL;
