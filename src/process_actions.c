...
void lsm_process_record_set(LsmApp *app, gboolean active)
{
    ...
    if (!lsm_home_directory(home, sizeof(home)) ||
        !lsm_join_path(directory, sizeof(directory),
                       home, LSM_LOG_DIRECTORY)) {
        lsm_ui_show_error(GTK_WINDOW(app->shell.window),
    ...
    const int mkdir_failure = lsm_mkdir_parents(directory, 0700U);
    if (mkdir_failure != 0) {
        lsm_ui_show_error(GTK_WINDOW(app->shell.window),
                          "Unable to create the log directory", "%s",
                          g_strerror(mkdir_failure));
        if (app->details.process_record_menu_item)
            gtk_check_menu_item_set_active(
                GTK_CHECK_MENU_ITEM(app->details.process_record_menu_item), FALSE);
        return;
    ...
    for (char *p = name; *p; p++)
        if (!lsm_ascii_is_alnum((unsigned char)*p) && *p != '-' && *p != '_') *p = '_';
...
void lsm_process_filters_load(LsmApp *app)
{
    ...
    char *content = NULL;
    size_t length = 0U;
    if (lsm_read_text_file_alloc(app->paths.filter_path, &content, &length) !=
        INFILTRATR_IO_OK)
        return;
    char *save = NULL;
    for (char *line = strtok_r(content, "\n", &save); line;
        ...
        lsm_trim(line);
        if (*line && *line != '#')
            g_ptr_array_add(app->process.filters, g_strdup(line));
    }
    free(content);
}
...
void lsm_process_filters_dialog(LsmApp *app)
{
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
        ...
        int failure = lsm_mkdir_parents(app->paths.config_dir, 0700U);
        if (failure == 0)
...
static void open_executable_directory(LsmApp *app)
{
    ...
    char directory[LSM_PATH_LEN];
    if (!lsm_path_dirname(process.executable, directory, sizeof(directory))) {
        lsm_ui_show_error(GTK_WINDOW(app->shell.window),
                          "Unable to open the executable directory",
                          "The executable directory path is too long.");
        return;
    }
    GError *error = NULL;
    char *uri = g_filename_to_uri(directory, NULL, &error);
    if (!uri || !gtk_show_uri_on_window(GTK_WINDOW(app->shell.window), uri,
                                        GDK_CURRENT_TIME, &error)) {
        lsm_ui_show_error(GTK_WINDOW(app->shell.window),
                          "Unable to open the executable directory", "%s",
