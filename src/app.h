...
/**
 * @file app.h
 * @brief GTK application state and cross-module user-interface API.
 *
 ...
 *
 * This callback owns the transition from GtkApplication lifetime to LsmApp
 * lifetime and must run on the GTK main thread.
 *
 * @param [in] application Active GtkApplication instance.
 * @param [in,out] user_data Application-owned activation context.
 */
void lsm_app_activate(GtkApplication *application, gpointer user_data);
/**
 * Stop timers, join in-process workers, close logs and release application state.
 *
 * @param [in,out] app Application to shut down; NULL is ignored.
 */
void lsm_app_shutdown(LsmApp *app);

/**
 * Recreate the fast presentation timer after graphical preferences change.
 *
 * This function must run on the GTK main thread. It does not perform an
 * immediate sample; callers decide which pages require synchronous refresh.
 *
 * @param [in,out] app Application whose timer policy changed.
 */
 ...
 *
 * The visible combo-box rows are explicitly mapped to metric identifiers, so
 * filtering unsupported backend-specific metrics cannot change selection
 * semantics. A hidden pre-sample graph receives a harmless Overall fallback
 * until detailed capabilities become available.
 *
