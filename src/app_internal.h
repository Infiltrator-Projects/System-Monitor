 ...
 */
#ifndef INFILTRATOR_SYSTEM_MONITOR_APP_INTERNAL_H
#define INFILTRATOR_SYSTEM_MONITOR_APP_INTERNAL_H
...
typedef struct {
    ...
    gboolean application_catalog_loading;
    gboolean page_built[LSM_TAB_COUNT];
    double page_scroll[LSM_TAB_COUNT];
    GtkWidget *page_scrollers[LSM_TAB_COUNT];
    GtkWidget *page_containers[LSM_TAB_COUNT];
...
typedef struct {
    ...
    GtkWidget *gauges[LSM_OVERVIEW_METRIC_COUNT];
...
typedef struct {
    ...
    gboolean startup_refresh_pending;
    gboolean startup_refresh_again;
    gboolean startup_action_pending;
} LsmStartupState;
...
/**
 * Lazily construct one notebook page after the shell has painted.
 *
 * This private composition hook is also used by navigation callbacks that may
 * reach a page before the background idle builder does.
 *
 * @param [in,out] app Application that owns the notebook and page state.
 * @param [in] page Notebook page that must be constructed before use.
 */
