...
static int remaining_timeout_ms(const struct timespec *deadline)
{
    ...
    if (lsm_posix_deadline_remaining_milliseconds(
            CLOCK_MONOTONIC, deadline, &remaining_ms) != 0)
        return LSM_HIDPP_IO_TIMEOUT_MS;
    return remaining_ms > (uint64_t)INT32_MAX
                          ...
                          int cancel_fd)
{
    ...
    if (lsm_posix_deadline_after_milliseconds(
            CLOCK_MONOTONIC, LSM_HIDPP_IO_TIMEOUT_MS, &deadline) != 0)
        return false;
