    ...
    while (count < capacity &&
        ...
        if (adapter) {
            ...
            (void)lookup_boolean(adapter, "Discovering",
                                 &record->discovering);
            if (!record->name[0])
                lsm_copy_string(record->name, sizeof(record->name), object_path);
    ...
    GVariant *objects, LsmBluetoothDeviceRecord *records, size_t capacity)
{
    ...
    while (count < capacity &&
        ...
        if (!record->alias[0])
            lsm_copy_string(record->alias, sizeof(record->alias),
...
{
    uint64_t refresh_milliseconds = 0U;
    if (!lsm_u64_multiply_checked(
            (uint64_t)LSM_BLUEZ_REFRESH_SECONDS, UINT64_C(1000),
            &refresh_milliseconds))
        return;
    struct timespec deadline;
    if (lsm_posix_deadline_after_milliseconds(
            CLOCK_MONOTONIC, refresh_milliseconds, &deadline) != 0)
        return;
