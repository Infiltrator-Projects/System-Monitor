...
static void read_process_cgroup(pid_t pid, LsmProcessInfo *process)
{
    if (!process) return;
    ...
    char path[128];
    char text[4096];
    (void)snprintf(path, sizeof(path), "/proc/%d/cgroup", pid);
    if (infiltratr_read_text_file_ex(path, text, sizeof(text), NULL) !=
        INFILTRATR_IO_OK)
        return;
    ...
    for (char *line = strtok_r(text, "\n", &save); line;
         line = strtok_r(NULL, "\n", &save)) {
        if (!lsm_string_starts_with(line, "0::")) continue;
        const char *group = line + 3U;
        if (!*group) group = "/";
        if (strlen(group) >= sizeof(process->cgroup_path)) return;
        lsm_copy_string(process->cgroup_path, sizeof(process->cgroup_path),
                        group);
        process->cgroup_v2 = true;
        break;
    }
}
        ...
        if (sample) {
            ...
            if (same_process && total_delta > 0U &&
                lsm_u64_counter_delta(
                    cpu_ticks, sample->cpu_ticks, &process_delta)) {
...
void lsm_process_error_message(char *buffer, size_t size)
{
    ...
    lsm_copy_string(buffer, size, strerror(error));
}
